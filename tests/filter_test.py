...
class TestFilters(BaseTest):
    """
     Не работает def test_filter_additional(self, driver):  # 1.Фильтрация данных в таблице
        fil = Test_Filters(driver, self.main_url() + self.login_url())
        fil.open()
        fil.filter_additional()

     Не работает def test_filter_table3(self, driver):  # 2.Фильтрация данных в таблице
        fil = Test_Filters(driver, self.main_url() + self.login_url())
        fil.open()
        fil.filter_table3()
